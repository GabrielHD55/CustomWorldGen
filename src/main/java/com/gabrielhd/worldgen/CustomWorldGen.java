...
public class CustomWorldGen extends JavaPlugin {
    ...
    public static World createWorld(CustomWorldCreator creator) {
        ...
        if (world != null) {
            ...
            if (worlddata == null) {
                DedicatedServerProperties.WorldGenProperties worldGenProperties = new DedicatedServerProperties.WorldGenProperties(String.valueOf(creator.seed()), new JsonParser().parse(creator.generatorSettings()).getAsJsonObject(), creator.generateStructures(), creator.type().getName());
    ...
    private static ResourceKey<NoiseGeneratorSettings> createGeneratorSettingBase(ResourceLocation minecraftKey, GeneratorConfiguration generatorConfiguration) {
        ...
        try {
            Method method = NoiseRouterData.class.getDeclaredMethod("overworld", Registry.class, boolean.class, boolean.class);
            method.setAccessible(true);

            noiseRouter = (NoiseRouter) method.invoke(NoiseRouterData.class, Registry.DENSITY_FUNCTION_REGISTRY, false, false);

            Field barrierField = noiseRouter.getClass().getDeclaredField("barrierNoise");
            barrierField.setAccessible(true);
            barrierField.set(noiseRouter, barrier);

            Field floodednessField = noiseRouter.getClass().getDeclaredField("fluidLevelFloodednessNoise");
            floodednessField.setAccessible(true);
            floodednessField.set(noiseRouter, fluid_level_floodedness);

            Field spreadField = noiseRouter.getClass().getDeclaredField("fluidLevelSpreadNoise");
            spreadField.setAccessible(true);
            spreadField.set(noiseRouter, fluid_level_spread);

            Field lavaField = noiseRouter.getClass().getDeclaredField("lavaNoise");
            lavaField.setAccessible(true);
            lavaField.set(noiseRouter, lavaField);

            Field temperatureField = noiseRouter.getClass().getDeclaredField("temperature");
            temperatureField.setAccessible(true);
            temperatureField.set(noiseRouter, temperature);

            Field vegetationField = noiseRouter.getClass().getDeclaredField("vegetation");
            vegetationField.setAccessible(true);
            vegetationField.set(noiseRouter, vegetation);

            Field continentsField = noiseRouter.getClass().getDeclaredField("continents");
            continentsField.setAccessible(true);
            continentsField.set(noiseRouter, continents);

            Field erosionField = noiseRouter.getClass().getDeclaredField("erosion");
